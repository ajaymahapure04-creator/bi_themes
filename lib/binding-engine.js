...
export function computeKpiCell(dataset, binding, activeFilters) {
  ...
  if (!table) return { label: binding.label || "KPI", value: "—", delta: null, isGood: null };
  ...
  return { label: binding.label || binding.metric.column, value: applyNumberFormat(value, binding.numberFormat), delta: null, isGood: null };
}
...
function emptyDataFor(type) {
  switch (type) {
    case "kpi": return { kpis: [{ label: "—", value: "—", delta: null, isGood: null }] };
    case "column":
    case "bar": return { bar: { title: "", cats: [], vals: [] } };
    case "line":
    case "area": return { line: { title: "", cats: [], s1: [], s2: [] } };
    case "columnGrouped": return { groupedBars: [{ title: "", cats: [], s1: [], s2: [] }] };
...
export function resolveCellData(type, binding, dataset, domainFallback, activeFilters) {
  ...
  try {
    switch (type) {
      ...
      // Same dual-series aggregation as line/area -- columnGrouped only
      // differs in how CellVisual renders it (grouped bars vs a trend line).
      // Wrapped in a single-element array to match groupedBars' shape so
      // idx%length always picks this one bound result (see CellVisual.jsx).
      case "columnGrouped": return { groupedBars: [computeLineCell(dataset, binding, activeFilters)] };
