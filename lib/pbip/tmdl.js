...
// A real Desktop-saved model.tmdl (see ota_dashboard_sample/) always closes
// with one `ref table <name>` per table and a `ref cultureInfo <locale>` --
// without them the table/culture .tmdl files exist on disk but aren't wired
// into the model at all, and Desktop fails to load the semantic model.
function modelTmdl(tableNames, culture) {
  const refTables = tableNames.map((n) => `ref table ${q(n)}`).join("\n");
  return [
    `model Model`,
    `\tculture: ${culture}`,
    ...
    ``,
    `annotation PBI_QueryOrder = [${tableNames.map((n) => `"${n.replace(/"/g, '\\"')}"`).join(", ")}]`,
    ``,
    refTables,
    ``,
...
export function buildSemanticModelFiles(plan, projectName) {
  ...
  const tableNames = [...plan.tables.values()].map((t) => t.name);
  ...
  files[`${root}/definition/model.tmdl`] = modelTmdl(tableNames, "en-US");
  files[`${root}/definition/cultures/en-US.tmdl`] = `cultureInfo en-US\n\n`;
  ...
  if (plan.relationships.length) {
    files[`${root}/definition/relationships.tmdl`] = plan.relationships
      .map((r, i) => [
        `relationship r${i + 1}`,
        // TMDL metadata column refs are dotted (Table.Column), not DAX's
        // bracket syntax ('Table'[Column]) -- using brackets here parses the
        // whole thing as one malformed object name and Desktop rejects the
        // file on open ("single-quote character in name must be escaped").
        `\tfromColumn: ${q(r.fromTable)}.${q(r.fromColumn)}`,
        `\ttoColumn: ${q(r.toTable)}.${q(r.toColumn)}`,
        "",
      ].join("\n"))
      .join("\n");
  }
