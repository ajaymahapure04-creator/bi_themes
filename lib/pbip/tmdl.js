...
const M_TYPE = { int64: "Int64.Type", double: "type number", string: "type text" };
...
// source to reconnect, no refresh credentials needed. The #table is explicitly
// typed (type table [Col = Int64.Type, ...]) so the source values arrive as
// real numbers, not the untyped "any" that Power BI would otherwise treat as
// text regardless of the TMDL column's declared dataType.
function mLiteralTable(columns, rows, effTypes) {
  const typeList = columns.map((c) => `#"${c.name.replace(/"/g, '""')}" = ${M_TYPE[effTypes[c.name]]}`).join(", ");
  ...
  return `let\n    Source = #table(type table [${typeList}], {${rowLits.join(",\n        ")}})\nin\n    Source`;
}
