...
function inferCategoryLabel(kind, title) {
  ...
  if (kind === "line" || kind === "area" || kind === "columnGrouped") return "Period";
...
function planCell(plan, { cell, i, dataset, d, activeFilters, geom }) {
  ...
  if (cell.type === "line" || cell.type === "area" || cell.type === "columnGrouped") {
    ...
    if (bound) {
      ...
      [m1, m2] = planCategorySeries(plan, `Cell ${i + 1} — ${r.title || "Trend"}`, r.cats, [
        ...
        // column sharing one name in the same table. "Actual"/"Target" is the
        // line/area default, matching the legend CellVisual.jsx renders for
        // that chart; a groupedBars entry carries its own real series names
        // (e.g. "Active users"/"New users") which take priority when present.
        { colName: "Series 1", vals: r.s1, label: r.s1Label || "Actual" },
        { colName: "Series 2", vals: r.s2, label: r.s2Label || "Target" },
      ], catLabel);
  ...
  if (cell.type === "table") {
    ...
    const r = unwrapResolved("table", resolved, i);
...
export function buildExportPlan({ theme, layout, domainKey, dataset }) {
  ...
  if (p.strip) {
    // One slot per actual KPI card (see buildLayoutSpec) -- not a fixed 4, or a
    // 7-card web-analytics-style domain would silently lose cards 5-7 from the export.
    Array.from({ length: d.kpis.length }, (_, i) => layout.kpiStripBindings?.[i] ?? null).forEach((binding, i) => {
  ...
  if (band && filters.length) {
    ...
    filters.forEach((f, i) => {
      ...
      if (!tableName) return;
      // Pull in whichever fact/dim table this filter's table is related to,
      // plus the relationship itself -- otherwise a slicer on a dimension
      // table lands in the model with no relationship at all, so it can't
      // actually cross-filter anything (and the relationship the user set up
      // for it silently never makes it into relationships.tmdl).
      dataset.relationships.forEach((r) => {
        if (r.factTable === f.table && ensureRealTable(plan, dataset, r.dimTable)) {
          ensureRealRelationship(plan, dataset, r.factTable, r.dimTable);
        } else if (r.dimTable === f.table && ensureRealTable(plan, dataset, r.factTable)) {
          ensureRealRelationship(plan, dataset, r.factTable, r.dimTable);
        }
      });
      const geom = vertical
