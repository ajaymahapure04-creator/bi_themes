...
function planCell(plan, { cell, i, dataset, d, activeFilters, geom }) {
  ...
  if (cell.type === "line" || cell.type === "area") {
    ...
    if (bound) {
      ...
      [m1, m2] = planCategorySeries(plan, `Cell ${i + 1} — ${r.title || "Trend"}`, r.cats, [
        { colName: "Series 1", vals: r.s1, label: "Series 1" },
        { colName: "Series 2", vals: r.s2, label: "Series 2" },
      ]);
  ...
  if (cell.type === "table") {
    ...
    const columns = r.cols.map((c) => ({ name: c, type: "string" }));
    const rows = r.rows.map((row) => Object.fromEntries(r.cols.map((c, ci) => [c, String(row[ci] ?? "")])));
