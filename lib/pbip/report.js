...
function visualJson(v, name) {
  ...
  if (v.kind === "kpi") {
    ...
    Object.assign(objects, titleObjects(v.label));
    ...
    Object.assign(objects, titleObjects(v.title));
  ...
  return {
    "$schema": "https://developer.microsoft.com/json-schemas/fabric/item/report/definition/visualContainer/2.0.0/schema.json",
    name, position,
    visual: { visualType: VISUAL_TYPE[v.kind], query: { queryState }, objects },
  };
}
...
export function buildReportFiles(plan, theme, projectName) {
  ...
  files[`${root}/definition/report.json`] = JSON.stringify({
    ...
    resourcePackages: [{ name: themeName, type: "CustomTheme", items: [{ type: "CustomTheme", path: `${themeName}.json`, name: themeName }] }],
  }, null, 2);
