...
const VISUAL_TYPE = {
  ...
  // Real Power BI visualType for a 2-series grouped bar chart -- confirmed
  // against the WebAnalytics.pbip reference (its chartL/chartR visuals).
  columnGrouped: "clusteredColumnChart",
};
// Which query "well" each visual's primary measure(s) go in.
const MEASURE_WELL = { kpi: "Data", column: "Y", bar: "Y", line: "Y", area: "Y", donut: "Y", table: "Values", columnGrouped: "Y" };
...
// format is optional and only carried by card/KPI projections in the
// reference (one decimal, "0.0") -- chart Y-well projections there have no
// format key at all, so it's a param rather than a blanket default.
const measureField = (table, measure, format) => ({
...
function visualJson(v, name) {
  ...
  if (v.kind === "kpi") {
    queryState[well] = { projections: [measureField(v.measure.tableName, v.measure.measureName, "0.0")] };
  ...
  } else if (["line", "area", "columnGrouped"].includes(v.kind)) {
    ...
    // Grouped mode mixes a real dimension column with real measures in the
    // same Values well -- exactly how Power BI's own table visual groups by
    // whatever column is present and aggregates whatever measures sit
    // alongside it. Raw/synthetic modes are plain column refs off one table.
    if (v.mode === "grouped") {
      const projections = [];
      if (v.groupByCol) projections.push(columnField(v.groupByTableName, v.groupByCol));
      v.measures.forEach((m) => projections.push(measureField(m.tableName, m.measureName)));
      queryState[well] = { projections };
    } else {
  ...
  const autoTitle = ["column", "bar", "donut", "line", "area", "columnGrouped"].includes(v.kind);
...
// Dropdown mode (not Desktop's default vertical checkbox list) + a 10pt
// header, matched to the reference's manually-formatted slicers -- keeps
// every slicer's header text the same size as every other visual's title
// instead of Desktop's larger default, and Dropdown keeps a filter bar of
// several slicers compact instead of each one growing into a checkbox list.
function slicerVisualJson(s, name, zOrder) {
  return {
    ...
    visual: {
      ...
      objects: {
        data: [{ properties: { mode: strLit("Dropdown") } }],
        header: [{ properties: { textSize: numLit(10) } }],
      },
      drillFilterOtherVisuals: true,
    },
  };
