...
export function buildLayoutSpec(layout, domainKey) {
  ...
  let kpiStrip = null;
  if (p.strip) {
    // Strip shows every one of the domain's KPI cards (see ReportPreview's
    // resolvedKpiStrip), not a fixed 4 -- a web-analytics-style domain with 7
    // cards needs 7 slots of export geometry too, or Order would silently drop
    // cards 5-7 from the PBIP/layout-spec even though the live preview shows them.
    const n = DOMAINS[domainKey].kpis.length;
    const kw = Math.floor((contentW - (n - 1) * G) / n);
    kpiStrip = Array.from({ length: n }, (_, i) => ({
